...
class Simulation:
    ...
    def run(self):
        ...
        try:
            ...
            # Graph 1: Path Length per Vehicle
            fig, ax = plt.subplots()
            df.plot(x="vehicle", y="path_length", kind="bar", ax=ax, legend=False)
            ax.set_ylabel("Path Length")
            ax.set_title("Path Length per Vehicle")
            fig.tight_layout()
            plt.savefig(os.path.join(self.cfg.output_dir, "graph_path_length.png"))
            if self.cfg.visualize: plt.show()

            # Graph 2: Planning Time per Vehicle
            fig, ax = plt.subplots()
            df.plot(x="vehicle", y="planning_time_s", kind="bar", ax=ax, legend=False, color='orange')
            ax.set_ylabel("Planning Time (s)")
            ax.set_title("Planning Time per Vehicle")
            fig.tight_layout()
            plt.savefig(os.path.join(self.cfg.output_dir, "graph_planning_time.png"))
            if self.cfg.visualize: plt.show()

            # Graph 3: Inflection Points
            fig, ax = plt.subplots()
            df.plot(x="vehicle", y="inflection_points", kind="bar", ax=ax, legend=False, color='green')
            ax.set_ylabel("Inflection Points")
            ax.set_title("Turns per Vehicle Path")
            fig.tight_layout()
            plt.savefig(os.path.join(self.cfg.output_dir, "graph_inflection_points.png"))
            if self.cfg.visualize: plt.show()

            # Graph 4: Total Turning Angle
            fig, ax = plt.subplots()
            df.plot(x="vehicle", y="total_turning_angle", kind="bar", ax=ax, legend=False, color='purple')
            ax.set_ylabel("Turning Angle (°)")
            ax.set_title("Total Turning Angle per Vehicle")
            fig.tight_layout()
            plt.savefig(os.path.join(self.cfg.output_dir, "graph_turning_angle.png"))
            if self.cfg.visualize: plt.show()

            # Graph 5: Federated Learning Effect - Weights Distribution
            fig, ax = plt.subplots()
            weights = [v.model.w.mean() for v in self.vehicles]
            ax.bar(range(len(weights)), weights, color='skyblue')
            ax.set_xlabel("Vehicle ID")
            ax.set_ylabel("Mean Model Weight")
            ax.set_title("Model Weight Distribution After FL")
            fig.tight_layout()
            plt.savefig(os.path.join(self.cfg.output_dir, "graph_federated_weights.png"))
            if self.cfg.visualize: plt.show()

            # Graph 6: Privacy-Preserving Effect (Noise Impact)
            noises = [self.cfg.dp_noise for _ in self.vehicles]
            fig, ax = plt.subplots()
            ax.bar(range(len(noises)), noises, color='salmon')
            ax.set_xlabel("Vehicle ID")
            ax.set_ylabel("DP Noise Level")
            ax.set_title("Privacy Noise Injected per Vehicle")
            fig.tight_layout()
            plt.savefig(os.path.join(self.cfg.output_dir, "graph_privacy_noise.png"))
            if self.cfg.visualize: plt.show()

        except Exception as e:
            logger.warning("Could not generate one or more analysis graphs: %s", e)
